...
import { 
  Leaf, Droplets, Wind, Settings, ChevronDown, Zap, Plus, Minus, Globe,
  Flower2, Activity, Check, Home, TestTube2, Calendar, MapPin, Moon, Sun
} from 'lucide-react';
...
const DEFAULT_CHART_DATA = Array.from({ length: 11 }).map((_, i) => ({
  day: i, normal: 100, biopot: 100, name: `${i}d`,
}));

type AppView = 'home' | 'realtime' | 'simulation' | 'camera';

// ─── CustomNumberInput (defined at module level — stable reference) ───────────
const CustomNumberInput = ({
  label, value, unit, onChange, min = 0, max = 100, step = 1,
}: {
  ...
  const adjust = useCallback((dir: 1 | -1) => {
    onChange(prev => {
      const next = (parseFloat(prev) || 0) + dir * step;
      return String(Math.min(max, Math.max(min, parseFloat(next.toFixed(2)))));
    });
  ...
  const startHold = useCallback((dir: 1 | -1) => {
    ...
    timerRef.current = setTimeout(() => {
      repeatRef.current = setInterval(() => adjust(dir), 100);
    }, 450);
  }, [adjust]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        ><Minus size={20} className="stroke-[3] sm:size-6" /></button>

        <div className="flex-1 relative h-16 sm:h-20">
          <input type="text" value={value}
            onChange={e => { const v = e.target.value; if (v === '' || v === '-' || /^-?\d*\.?\d*$/.test(v)) onChange(v); }}
            onBlur={() => { const p = parseFloat(value); onChange(isNaN(p) ? '0' : String(Math.min(max, Math.max(min, p)))); }}
          ...
          />
          <span className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 text-[8px] sm:text-[10px] font-black text-slate-300 pointer-events-none uppercase tracking-tighter">{unit}</span>
        </div>
        ...
        ><Plus size={20} className="stroke-[3] sm:size-6" /></button>
      </div>
    </div>
  );
};

// ─── CustomDropdown (defined at module level — FIXES removeChild crash) ───────
// CRITICAL: must be a stable top-level component, NOT defined inside App or
// any render function. Defining it inside causes React to treat it as a new
// component type on every parent re-render, forcing full unmount/mount which
// breaks AnimatePresence exit animations → removeChild crash.
const CustomDropdown = ({
  label, value, options, isOpen, setIsOpen, onSelect, placeholder, icon: Icon,
}: {
  label: string; value: string | null; options: { id: string; label: string }[];
  isOpen: boolean; setIsOpen: (v: boolean) => void;
  onSelect: (id: string) => void; placeholder: string; icon: React.ElementType;
}) => (
  <div className="relative w-full space-y-2">
    ...
    <button
      ...
      className={`w-full px-6 py-4 rounded-2xl border-2 transition-all flex items-center justify-between text-left ${
        isOpen ? 'border-[#2e7d32] bg-white ring-4 ring-green-100' : 'border-transparent bg-white/50 hover:bg-white'
      }`}
    >
      <span className={`font-bold ${value ? 'text-slate-900' : 'text-slate-400'}`}>{value || placeholder}</span>
      <ChevronDown size={18} className={`transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
    </button>

    {/* Use CSS visibility instead of AnimatePresence to avoid removeChild */}
    <div className={`absolute z-[100] w-full mt-2 bg-white rounded-[1.5rem] shadow-2xl border border-green-50 overflow-hidden max-h-[250px] overflow-y-auto transition-all duration-200 ${
      ...
      {options.map(opt => (
        ...
        >
          <span className={`font-bold ${value === opt.label ? 'text-[#2e7d32]' : 'text-slate-600'}`}>{opt.label}</span>
...
function App() {
  ...
  const [selectedCoords, setSelectedCoords] = useState<{ lat: number; lon: number } | null>({ lat: 10.7626, lon: 106.6601 });
  ...
  useEffect(() => {
    axios.get('/api/plants').then(r => setPlants(r.data)).catch(() => {});
    const handler = (e: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
        setIsGroupOpen(false); setIsPlantOpen(false); setIsPotOpen(false); setShowSuggestions(false);
      }
    };
  ...
  // Geo autocomplete
  useEffect(() => {
    if (isSelectionMade.current) { isSelectionMade.current = false; setSuggestions([]); return; }
    if (location.trim().length < 2) { setSuggestions([]); return; }
    const t = setTimeout(async () => {
      setIsSearching(true);
      try { const r = await axios.get(`/api/geo?q=${encodeURIComponent(location)}`); setSuggestions(r.data); }
      catch {}
  ...
  const handleResetState = useCallback(() => {
    setResults(null); setSelectedGroup(null); setSelectedPlantId(null); setSelectedPotId(null);
    setLocation('Ho Chi Minh City, VN'); setSelectedCoords({ lat: 10.7626, lon: 106.6601 });
    setManualTemp('0'); setManualHumidity('0'); setManualWind('0');
    setWeather(null); setError(null); setLoading(false); setResetKey(k => k + 1);
  }, []);
  ...
  const fetchDataAndCalculate = useCallback(async () => {
    ...
    try {
      let Tmax: number, Tmin: number, humidity: number, wind: number;
      if (view === 'realtime') {
        try {
          const url = selectedCoords
            ? `/api/weather?lat=${selectedCoords.lat}&lon=${selectedCoords.lon}`
            : `/api/weather?city=${encodeURIComponent(location)}`;
          const r = await axios.get(url);
          const w = r.data;
          Tmax = w.main.temp_max; Tmin = w.main.temp_min;
          humidity = w.main.humidity; wind = w.wind.speed;
          setWeather(w);
        } catch {
          setError(t('weather_error')); setLoading(false); return;
        }
      } else {
        const tVal = parseFloat(manualTemp) || 0;
        Tmax = tVal; Tmin = tVal;
        humidity = parseFloat(manualHumidity) || 0;
        wind     = parseFloat(manualWind)     || 0;
      }
      const r = await axios.post('/api/calculate', { plantId: selectedPlantId, pot_type: selectedPotId, Tmax, Tmin, humidity, wind, time: 0 });
      setResults(r.data);
    } catch { /* silent */ }
    finally { setLoading(false); }
  }, [selectedPlantId, selectedPotId, view, selectedCoords, location, manualTemp, manualHumidity, manualWind, t]);

  const filteredPlants = useMemo(() => plants.filter(p => p.group === selectedGroup), [plants, selectedGroup]);
  const selectedPlant  = useMemo(() => plants.find(p => p.id === selectedPlantId),   [plants, selectedPlantId]);

  const processedChartData = useMemo(() => {
    const potKey  = POT_TYPES.find(p => p.id === selectedPotId)?.key || 'pot_plastic';
    const potLabel  = t(potKey);
    const bioLabel  = t('pot_biopot');
    if (!results?.chart_data) {
      return DEFAULT_CHART_DATA.map(d => ({ name: `${d.day}${t('days_unit')}`, [potLabel]: 100, [bioLabel]: 100 }));
    }
    return results.chart_data.map((d: any) => ({
      name: `${d.day}${t('days_unit')}`,
      [potLabel]: Number(d.normal.toFixed(1)),
      [bioLabel]: Number(d.biopot.toFixed(1)),
    }));
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-green-200 ${darkMode ? 'bg-[#0d1117] text-slate-100' : 'bg-beige text-slate-800'}`}>
      ...
      <nav className={`fixed top-0 w-full z-[100] backdrop-blur-md border-b flex justify-between items-center px-4 sm:px-6 md:px-12 py-3 sm:py-5 ${darkMode ? 'bg-[#0d1117]/90 border-white/10' : 'bg-white/80 border-white/20'}`}>
        <button onClick={() => handleViewChange('home')} className="flex items-center gap-2 sm:gap-3 hover:opacity-80 transition-opacity">
        ...
        <div className="hidden lg:flex items-center gap-2">
          {([['home', 'home', Home], ['realtime', 'realtime_nav', Leaf], ['simulation', 'simulation_nav', TestTube2]] as const).map(([id, key, Icon]) => (
            <button key={id} onClick={() => handleViewChange(id as AppView)}
              className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === id ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}
            ><Icon size={16} />{t(key)}</button>
          ))}
          <button onClick={() => handleViewChange('camera')}
            className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === 'camera' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}
          >📷 {i18n.language === 'vi' ? 'Quét AI Vision' : 'AI Vision Scan'}</button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >{darkMode ? <Sun size={16} className="text-yellow-400" /> : <Moon size={16} className="text-slate-500" />}</button>

          <button onClick={() => i18n.changeLanguage(i18n.language === 'en' ? 'vi' : 'en')}
            className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border font-bold text-xs sm:text-sm transition-all shadow-sm ${darkMode ? 'bg-[#1a2332] border-white/10 text-slate-200 hover:bg-[#243044]' : 'bg-white border-white text-slate-700 hover:bg-green-50'}`}
          ><Globe size={16} className="text-primary" /><span>{i18n.language === 'en' ? 'English' : 'Tiếng Việt'}</span><ChevronDown size={12} className="opacity-30" /></button>
        </div>
      </nav>

      {/* ── Mobile bottom nav ── */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-xl border-t border-slate-100 px-6 py-3 flex justify-around items-center rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        {([
          ['home',       'home',           Home],
          ['realtime',   'realtime_nav',   Globe],
          ['simulation', 'simulation_nav', TestTube2],
        ] as const).map(([id, key, Icon]) => (
          <button key={id} onClick={() => handleViewChange(id as AppView)}
            className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === id ? 'text-primary' : 'text-slate-400'}`}
          >
            <Icon size={22} className={view === id ? 'fill-primary/20' : ''} />
            <span className="text-[10px] font-black uppercase tracking-widest">{t(key)}</span>
            {/* plain div dot — no layoutId to avoid removeChild */}
            <div className={`w-1 h-1 rounded-full bg-primary mt-0.5 transition-opacity duration-200 ${view === id ? 'opacity-100' : 'opacity-0'}`} />
          </button>
        ))}
        <button onClick={() => handleViewChange('camera')}
          className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'camera' ? 'text-primary' : 'text-slate-400'}`}
        >
      ...
      {/* ── Main content (home / realtime / simulation) ── */}
      {/* camera view is rendered separately below — do NOT include it here */}
      {view !== 'camera' && (
        <AnimatePresence mode="wait">
          {view === 'home' ? (
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-center mb-12 sm:mb-16 relative">
                <div className="space-y-6 sm:space-y-10 z-10 text-center lg:text-left">
                  ...
                  <p className="text-lg sm:text-xl text-slate-700 font-medium leading-relaxed opacity-80 px-4 sm:px-0 max-w-xl mx-auto lg:mx-0">
                    {t('welcome_desc')}
                  </p>
                </div>
                ...
                <div className="relative h-[400px] sm:h-[450px] lg:h-[650px] w-full flex items-center justify-center lg:justify-end overflow-hidden lg:overflow-visible scale-90 sm:scale-100">
                  <div className="absolute right-1/2 translate-x-1/2 lg:translate-x-0 lg:right-0 top-1/2 -translate-y-1/2 w-[350px] sm:w-[550px] h-[350px] sm:h-[550px] z-10">
                    <div className="w-full h-full border-[6px] sm:border-[10px] border-white shadow-2xl shadow-green-900/10 overflow-hidden"
                      style={{ borderRadius: '30% 70% 70% 30% / 30% 30% 70% 70%' }}>
                      <img src={handImg} alt="Planting" className="w-full h-full object-cover" />
                    </div>
                  </div>
                  <div className="absolute -top-4 sm:top-10 -right-4 sm:right-4 w-[160px] sm:w-[240px] h-[160px] sm:h-[240px] rounded-full border-[6px] sm:border-[8px] border-white shadow-2xl overflow-hidden z-20">
                    <img src={humanImg} alt="Gardener" className="w-full h-full object-cover" />
                  </div>
                  <div className="absolute bottom-4 sm:bottom-1/4 left-0 sm:left-10 w-[140px] sm:w-[200px] h-[140px] sm:h-[200px] rounded-full border-[6px] sm:border-[8px] border-white shadow-2xl overflow-hidden z-20">
                    <img src={leafImg} alt="Seedling" className="w-full h-full object-cover" />
                  </div>
                </div>
              </div>

              <div className="bg-white/95 backdrop-blur-xl p-3 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex flex-col md:flex-row items-stretch mx-2 sm:mx-4">
                {[
                  { icon: Droplets, t1: 'feat1_title', t2: 'feat1_desc' },
                  { icon: Leaf,     t1: 'feat2_title', t2: 'feat2_desc' },
                  { icon: Calendar, t1: 'feat3_title', t2: 'feat3_desc' },
                ].map(({ icon: Icon, t1, t2 }, i) => (
                  <React.Fragment key={t1}>
                    {i > 0 && <div className="hidden md:block w-px bg-slate-100 my-6" />}
                    <div className={`flex-1 flex gap-4 sm:gap-5 p-4 sm:p-6 items-center ${i > 0 ? 'border-t border-slate-50 md:border-t-0' : ''}`}>
                      <div className="p-3 sm:p-4 bg-[#f1f8e9] rounded-full flex-shrink-0 shadow-inner">
                        <Icon className="text-[#33691e]" size={24} />
                      </div>
                      <div className="space-y-0.5">
                        <h3 className="text-base sm:text-lg font-black text-[#1b3022] leading-tight">{t(t1)}</h3>
                        <p className="text-[10px] sm:text-xs font-bold text-slate-500 leading-snug max-w-[200px]">{t(t2)}</p>
                      </div>
                    </div>
                  </React.Fragment>
                ))}
            ...
            <motion.main key="calculator" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
                  ...
                  <div className="space-y-8">
                    {view === 'realtime' ? (
                      <div className="space-y-3 relative">
                        ...
                        {/* Suggestions — CSS transition, no AnimatePresence */}
                        <div className={`absolute z-[110] w-full mt-2 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden transition-all duration-200 ${showSuggestions && suggestions.length > 0 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                          {suggestions.map((s, idx) => (
                            <button key={`${s.name}-${s.lat}-${idx}`}
                              onClick={() => {
                                const name = `${s.name}${s.state && s.state !== s.name ? `, ${s.state}` : ''}, ${s.country}`;
                                isSelectionMade.current = true;
                                setLocation(name); setSelectedCoords({ lat: s.lat, lon: s.lon }); setShowSuggestions(false);
                              }}
                      ...
                      <div className="space-y-6" key={resetKey}>
                        <CustomNumberInput label={t('temp_label')}     value={manualTemp}     min={-20} max={60}  unit="°C"  onChange={setManualTemp}     />
                        <CustomNumberInput label={t('humidity_label')} value={manualHumidity} min={0}   max={100} unit="%"   onChange={setManualHumidity} />
                        <CustomNumberInput label={t('wind_label')}     value={manualWind}     min={0}   max={100} unit="m/s" onChange={setManualWind} step={0.5} />
                      </div>
                    )}

                    <CustomDropdown label={t('plant_group_label')}
                      ...
                      isOpen={isGroupOpen}
                      setIsOpen={v => { setIsGroupOpen(v); setIsPlantOpen(false); setIsPotOpen(false); }}
                      onSelect={id => { setSelectedGroup(id); setSelectedPlantId(null); }}
                      icon={Plus}
                    />

                    <CustomDropdown label={t('plant_label')}
                      ...
                      isOpen={isPlantOpen}
                      setIsOpen={v => { setIsPlantOpen(v); setIsGroupOpen(false); setIsPotOpen(false); }}
                      onSelect={id => setSelectedPlantId(id)}
                      icon={Flower2}
                    />

                    <CustomDropdown label={t('pot_label')}
                      ...
                      isOpen={isPotOpen}
                      setIsOpen={v => { setIsPotOpen(v); setIsGroupOpen(false); setIsPlantOpen(false); }}
                    ...
                    >
                      {loading ? (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="animate-pulse">{results ? t('updating_label') : t('calculating_label')}</span>
                        </div>
                      ) : (
                        <><Zap size={24} fill={results ? 'none' : 'currentColor'} className={results ? 'animate-bounce' : ''} />
                        {results ? t('update_btn') : t('calculate_btn')}</>
                      )}
                    </motion.button>
                    ...
                    ><Activity size={14} />{t('reset_btn')}</button>

                    {/* Error — CSS transition, no AnimatePresence */}
                    <div className={`overflow-hidden transition-all duration-300 ${error ? 'max-h-24 opacity-100' : 'max-h-0 opacity-0'}`}>
                      <div className="p-4 bg-red-50 rounded-2xl text-red-500 text-[10px] font-bold uppercase tracking-widest text-center border border-red-100">
                        {error}
                      </div>
                    </div>
                  </div>
                ...
                {selectedPlant && (
                  <div className="bg-[#1b3022] p-8 rounded-[2.5rem] text-white overflow-hidden relative shadow-2xl">
                    <div className="relative z-10 space-y-4">
                      ...
                      <h3 className="text-3xl font-black">{i18n.language === 'en' ? selectedPlant.name_en : selectedPlant.name_vi}</h3>
              ...
              <div className="col-span-12 lg:col-span-8 space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-white flex flex-col justify-between min-h-[180px] sm:min-h-[220px]">
                    <div className="flex justify-between items-start">
                      <div className="p-3 sm:p-4 bg-blue-50 rounded-2xl"><Droplets className="text-blue-500" size={24} /></div>
                  ...
                  <div className="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-white flex flex-col justify-between min-h-[180px] sm:min-h-[220px]">
                    <div className="flex justify-between items-start">
                      <div className="p-3 sm:p-4 bg-orange-50 rounded-2xl"><Wind className="text-orange-500" size={24} /></div>
                  ...
                  <div className="bg-primary p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl text-white flex flex-col justify-between min-h-[180px] sm:min-h-[220px] relative overflow-hidden">
                    <div className="relative z-10 flex justify-between items-start">
                      <div className="p-3 sm:p-4 bg-white/20 rounded-2xl"><Calendar className="text-white" size={24} /></div>
                    ...
                    <Leaf className="absolute -right-6 -bottom-6 sm:-right-10 sm:-bottom-10 opacity-10 rotate-12" size={120} />
                  </div>
                </div>
                ...
                {weather && results && (
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {[
                        { icon: Activity, color: 'red',    val: `${weather.main.temp.toFixed(1)}°C`, label: t('temp_label'),     sub: weather.main.feels_like ? `(${t('feels_like')} ${weather.main.feels_like.toFixed(0)}°)` : undefined },
                        { icon: Droplets, color: 'blue',   val: `${weather.main.humidity}%`,         label: t('humidity_label'), sub: undefined },
                        { icon: Wind,     color: 'orange', val: `${weather.wind.speed.toFixed(1)} m/s`, label: t('wind_label'), sub: undefined },
                      ].map(({ icon: Icon, color, val, label, sub }) => (
                        <div key={label} className="flex items-center gap-4 p-8 bg-white rounded-[2.5rem] shadow-xl border border-white">
                          <div className={`p-4 bg-${color}-50 rounded-2xl text-${color}-500`}><Icon size={24} /></div>
                          <div>
                            ...
                            <p className="text-2xl font-black text-[#1b3022]">{val}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="h-[300px] sm:h-[450px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={processedChartData} margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
                        ...
                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 800, fill: '#94a3b8' }} dy={20} />
                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 800, fill: '#94a3b8' }} unit="%" domain={[0, 100]} />
                        <Tooltip contentStyle={{ borderRadius: '24px', border: 'none', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.15)', padding: '20px', fontWeight: 900, backgroundColor: 'white' }} />
                        <Line type="monotone" dataKey={t(POT_TYPES.find(p => p.id === (selectedPotId || 'plastic'))?.key || 'pot_plastic')}
                          stroke="#ef4444" strokeWidth={5} dot={{ r: 6, fill: '#ef4444', strokeWidth: 3, stroke: '#fff' }} activeDot={{ r: 10 }} />
                        <Line type="monotone" dataKey={t('pot_biopot')}
                          stroke="#2e7d32" strokeWidth={5} dot={{ r: 6, fill: '#2e7d32', strokeWidth: 3, stroke: '#fff' }} activeDot={{ r: 10 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                    ...
                    <div className="bg-primary/5 p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] border border-primary/10 flex flex-col md:flex-row items-center gap-6 sm:gap-10">
                      ...
                      <div className="flex-1 text-center md:text-left space-y-2">
                        ...
                        <p className="font-bold text-[#1b3022] text-lg sm:text-xl leading-snug">
                          {t('insight_text', { percentage: results.comparison.water_saved_percent, potType: t(POT_TYPES.find(p => p.id === selectedPotId)?.key || '') })}
                        </p>
                      </div>
      ...
      {/* ── Camera View (fullscreen fixed overlay) ── */}
      {view === 'camera' && (
        <div className="fixed inset-0 z-[50] pt-[60px] lg:pt-[72px]">
          <ErrorBoundary darkMode={darkMode}>
            <CameraScanner darkMode={darkMode} lang={i18n.language} />
          </ErrorBoundary>
        </div>
      )}
        ...
        <footer className={`py-16 border-t ${darkMode ? 'bg-[#0d1117] border-white/5' : 'bg-white border-slate-50'}`}>
          <div className="max-w-[1400px] mx-auto px-6 flex flex-col items-center gap-8">
            ...
            <div className="flex gap-8 text-primary opacity-20">
              <Settings size={20} /><Plus size={20} /><Globe size={20} />
            </div>
          </div>
        </footer>
      )}

...
const CustomNumberInput = ({
  ...
  const handleBlur = () => {
    ...
    else onChange(String(Math.min(max, Math.max(min, parsed))));
  };
...
// FIX 1: CustomDropdown định nghĩa BÊN TRONG App để tránh re-mount toàn bộ
// khi App re-render (dark mode / language toggle) gây removeChild crash.
// Dùng CSS visibility thay vì AnimatePresence để tránh unmount DOM node.
function App() {
  ...
  // FIX 1: CustomDropdown định nghĩa bên trong App — KHÔNG dùng AnimatePresence.
  // Thay bằng CSS transition + visibility để tránh unmount/removeChild crash.
  const CustomDropdown = ({
    ...
    <div className="relative w-full space-y-2">
      ...
      {/* CSS-only show/hide — no AnimatePresence, no motion.div → no removeChild crash */}
      <div
        className={`absolute z-[100] w-full mt-2 bg-white rounded-[1.5rem] shadow-2xl border border-green-50 overflow-hidden max-h-[250px] overflow-y-auto transition-all duration-200 ${
    ...
    try {
      ...
      if (view === 'realtime') {
        try {
          ...
          if (selectedCoords) url = `/api/weather?lat=${selectedCoords.lat}&lon=${selectedCoords.lon}`;
      ...
      const calcRes = await axios.post('/api/calculate', {
        plantId: selectedPlantId, pot_type: selectedPotId,
        Tmax: currentTmax, Tmin: currentTmin,
        humidity: currentHumidity, wind: currentWind, time: 0
      });
  ...
  // FIX 2: toggleLanguage — force re-render sau khi đổi ngôn ngữ
    ...
    <div className={`min-h-screen font-sans selection:bg-green-200 ${darkMode ? 'bg-[#0d1117] text-slate-100' : 'bg-beige text-slate-800'}`}>
      ...
      <nav className={`fixed top-0 w-full z-[100] backdrop-blur-md border-b flex justify-between items-center px-4 sm:px-6 md:px-12 py-3 sm:py-5 ${darkMode ? 'bg-[#0d1117]/90 border-white/10' : 'bg-white/80 border-white/20'}`}>
        ...
        <div className="hidden lg:flex items-center gap-2">
          <button onClick={() => handleViewChange('home')} className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === 'home' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}>
            <Home size={16} />{t('home')}
          </button>
          <button onClick={() => handleViewChange('realtime')} className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === 'realtime' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}>
            <Leaf size={16} />{t('realtime_nav')}
          </button>
          <button onClick={() => handleViewChange('simulation')} className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === 'simulation' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}>
            <TestTube2 size={16} />{t('simulation_nav')}
          </button>
          {/* FIX 3: Nút AI Vision dùng t() thay vì hardcode */}
          <button onClick={() => handleViewChange('camera')} className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-2 ${view === 'camera' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-primary hover:bg-white/50'}`}>
            📷 {t('ai_vision_nav')}
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {darkMode ? <Moon size={16} className="text-yellow-400 fill-yellow-400" /> : <Sun size={16} className="text-yellow-500 fill-yellow-500" />}
          </button>
          <button
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-xl border-t border-slate-100 px-6 py-3 pb-safe-offset-2 flex justify-around items-center rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <button onClick={() => handleViewChange('home')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'home' ? 'text-primary' : 'text-slate-400'}`}>
        ...
        <button onClick={() => handleViewChange('realtime')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'realtime' ? 'text-primary' : 'text-slate-400'}`}>
        ...
        <button onClick={() => handleViewChange('simulation')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'simulation' ? 'text-primary' : 'text-slate-400'}`}>
        ...
        <button onClick={() => handleViewChange('camera')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'camera' ? 'text-primary' : 'text-slate-400'}`}>
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-center mb-12 sm:mb-16 relative">
              <div className="space-y-6 sm:space-y-10 z-10 text-center lg:text-left">
                ...
                <div className="relative pt-4">
                  <motion.div
                    initial={{ rotate: -15 }} animate={{ rotate: 15 }}
              ...
              <div className="relative h-[400px] sm:h-[450px] lg:h-[650px] w-full flex items-center justify-center lg:justify-end overflow-hidden lg:overflow-visible scale-90 sm:scale-100">
                <div className="absolute right-1/2 translate-x-1/2 lg:translate-x-0 lg:right-0 top-1/2 -translate-y-1/2 w-[350px] sm:w-[550px] h-[350px] sm:h-[550px] z-10">
                  <div className="w-full h-full border-[6px] sm:border-[10px] border-white shadow-2xl shadow-green-900/10 overflow-hidden" style={{ borderRadius: '30% 70% 70% 30% / 30% 30% 70% 70%' }}>
            ...
            <div className="bg-white/95 backdrop-blur-xl p-3 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex flex-col md:flex-row items-stretch relative z-20 mx-2 sm:mx-4">
              <div className="flex-1 flex gap-4 sm:gap-5 p-4 sm:p-6 items-center">
                <div className="p-3 sm:p-4 bg-[#f1f8e9] rounded-full flex-shrink-0 shadow-inner"><Droplets className="text-[#33691e]" size={24} /></div>
              ...
              <div className="flex-1 flex gap-4 sm:gap-5 p-4 sm:p-6 items-center border-t border-slate-50 md:border-t-0">
                <div className="p-3 sm:p-4 bg-[#f1f8e9] rounded-full flex-shrink-0 shadow-inner"><Leaf className="text-[#33691e]" size={24} fill="currentColor" /></div>
                <div className="space-y-0.5">
              ...
              <div className="hidden md:block w-px bg-slate-100 my-6" />
              <div className="flex-1 flex gap-4 sm:gap-5 p-4 sm:p-6 items-center border-t border-slate-50 md:border-t-0">
                <div className="p-3 sm:p-4 bg-[#f1f8e9] rounded-full flex-shrink-0 shadow-inner"><Calendar className="text-[#33691e]" size={24} /></div>
                <div className="space-y-0.5">
              ...
              <div className="bg-white/60 backdrop-blur-md p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl border border-white space-y-8 sm:space-y-10">
                ...
                <div className="space-y-8">
                  ...
                  <CustomDropdown
                    ...
                    isOpen={isGroupOpen}
                    setIsOpen={(val: boolean) => { setIsGroupOpen(val); setIsPlantOpen(false); setIsPotOpen(false); }}
                    onSelect={(id: string) => { setSelectedGroup(id); setSelectedPlantId(null); }}
                    icon={Plus}
                  />
                  ...
                  <CustomDropdown
                    ...
                    isOpen={isPlantOpen}
                    setIsOpen={(val: boolean) => { setIsPlantOpen(val); setIsGroupOpen(false); setIsPotOpen(false); }}
                  ...
                  <CustomDropdown
                    ...
                    isOpen={isPotOpen}
                    setIsOpen={(val: boolean) => { setIsPotOpen(val); setIsGroupOpen(false); setIsPlantOpen(false); }}
                  ...
                  <motion.button
                    ...
                    className={`w-full py-6 text-white rounded-[2rem] font-bold text-lg uppercase tracking-widest shadow-2xl transition-all flex items-center justify-center gap-3 relative z-10 ${results ? 'bg-orange-600 shadow-orange-900/40 hover:bg-orange-700' : 'bg-primary shadow-green-900/40 hover:bg-green-800'} disabled:opacity-50 disabled:grayscale`}
                  >
                    {loading ? (
                  ...
                  <button onClick={handleResetState} className="w-full py-4 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors flex items-center justify-center gap-2">
                    <Activity size={14} />{t('reset_btn')}
                  </button>
                    ...
                    <div className="flex items-center gap-4 p-8 bg-white rounded-[2.5rem] shadow-xl border border-white">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {weather.main.feels_like && <span className="text-[9px] font-bold text-slate-300">({t('feels_like')} {weather.main.feels_like.toFixed(0)}°)</span>}
                        </div>
                ...
                <div className="h-[300px] sm:h-[450px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={processedChartData} margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
                      ...
                      <Line type="monotone" dataKey={t(POT_TYPES.find(p => p.id === (selectedPotId || 'plastic'))?.key || 'pot_plastic')} stroke="#ef4444" strokeWidth={5} dot={{ r: 6, fill: '#ef4444', strokeWidth: 3, stroke: '#fff' }} activeDot={{ r: 10 }} />
                      <Line type="monotone" dataKey={t('pot_biopot')} stroke="#2e7d32" strokeWidth={5} dot={{ r: 6, fill: '#2e7d32', strokeWidth: 3, stroke: '#fff' }} activeDot={{ r: 10 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
      ...
      {view === 'camera' && (
        <div className="fixed inset-0 z-[50] pt-[60px] lg:pt-[72px]">
          <ErrorBoundary darkMode={darkMode}>
            {/* FIX 4: truyền language xuống CameraScanner để re-render khi đổi ngôn ngữ */}
            <CameraScanner darkMode={darkMode} language={i18n.language} />
          </ErrorBoundary>
        </div>
      )}
